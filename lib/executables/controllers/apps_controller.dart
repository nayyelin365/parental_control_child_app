...
class AppsController extends GetxController implements GetxService {
  ...
  saveChildId(childId) {
    prefs.setString(AppConstants.childId, childId);
    Get.find<MethodChannelController>().setPassword();
    log("${prefs.getString(AppConstants.childId)}", name: "save childId");
  }

  saveFirebaseToken(firebaseToken) {
    prefs.setString(AppConstants.firebaseToken, firebaseToken);
    Get.find<MethodChannelController>().setPassword();
    log("${prefs.getString(AppConstants.firebaseToken)}",
        name: "save firebaseToken");
  }

  getFirebaseToken() {
    return prefs.getString(AppConstants.firebaseToken) ?? "";
  }

  getChildId() {
    return prefs.getString(AppConstants.childId) ?? "";
  }
